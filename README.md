...
something may be hiding in the URL.** linklint turns that intuition into 31 deterministic
detectors, each emitting a named, documented reason code (one, the agent-mode
prompt-injection detector, is opt-in via `agentMode`).
...
linklint runs **31 offline detectors** grouped into the families below (the agent-mode
prompt-injection detector is opt-in via `agentMode` and off by default). Every example
...
| Path | What |
|------|------|
| `packages/core` | The `linklint` npm package — source of truth (`inspect()`, 31 detectors, scoring, policy, schema). |
