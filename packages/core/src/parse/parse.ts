...
function buildContext(input: string, raw: RawParts, runtime: RuntimeConfig): InspectionContext {
  ...
  return {
    ...
    hostLabels,
    registrableDomain: psl.registrableDomain,
    registrableDomainLower: psl.registrableDomain?.toLowerCase() ?? null,
    publicSuffix: psl.publicSuffix,
    publicSuffixTld: psl.publicSuffix ? psl.publicSuffix.split(".").pop()! : null,
    subdomain: psl.subdomain,
    subdomainLabels: psl.subdomain ? psl.subdomain.split(".") : [],
    port: raw.port,
    path: raw.path,
    query: raw.query,
    fragment: raw.fragment,
