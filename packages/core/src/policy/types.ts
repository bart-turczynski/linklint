...
/**
 * A single policy axis: a pure function that reads normalized policy config
 * from the inspection context and returns the layer-`policy` findings (weight
 * 0) the input violates for that axis. Axes are deterministic, synchronous, and
 * must never throw. The dispatcher in `policy.ts` iterates the configured axes
 * and concatenates their findings in a fixed order.
 */
