...
describe("file_extension_tld (J6) is the only TLD-shaped scoring signal", () => {
  it("a bare filename host flags file_extension_tld on the masquerade structure", () => {
...
// Converted from the `risky_tld` coverage row (LINK-brsntven). The claim the
// old test made — ".tk is a signal" — is exactly the world-claim that was
// deleted; what must hold now is the opposite, and it is worth asserting
// because it is what a future re-proposal would break.
describe("a curated high-abuse TLD is NOT a finding on its own", () => {
  it("a bare .tk host scores 0 with no reasons at all", () => {
  ...
  it("the caller's own deny-list is where that judgment lives, at weight 0", () => {
    const r = inspect("https://promo.tk/", { denyTlds: ["tk"] });
    expect(r.reasons.map((x) => x.code)).toEqual(["tld_denied"]);
    expect(r.score).toBe(0);
  });
});
