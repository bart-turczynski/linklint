...
## `boundary-baseline.json` — generated, not vendored

Not an upstream corpus: this is **linklint's own answers** for 255 hosts,
recorded so a `tldts` / `tr46` pin bump shows its blast radius before it merges.
Generated by [`../boundary-baseline.ts`](../boundary-baseline.ts) — regenerate
with `pnpm data:boundary`, never hand-edit. See `docs/architecture.md` §6.3 and
the bump checklist in `CONTRIBUTING.md`.
