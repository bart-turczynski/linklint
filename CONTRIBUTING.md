...
There are no scheduled pipelines (`LINK-txxcwplc`, declined): the dependency
audit is run by hand (§*Auditing dependencies for known vulnerabilities*).
...
A clock is the only trigger that could run this check unattended — an advisory
is disclosed against a lockfile that has not moved, so no `changes:` rule can
fire on it. The project declined one anyway (`LINK-txxcwplc`): it runs no
scheduled pipelines, pipeline variables are disabled, and runner minutes are
scarce. So the audit is yours to run, at the two moments above — before a
release (§*Cutting a release*) and after any dependency change.
...
> The project runs no scheduled pipelines (`LINK-txxcwplc`, declined), so it is
> yours to run.
>
